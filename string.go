...
func (e *Encoder) nonAscii(b byte) string {
	switch b {
	case '\\':
		return "\\"
	case '\a':
...
func (e *Encoder) encodeStringRaw(s string) error {
	last := 2
	copy(e.scratch[:], "s:")
	last = len(appendInt(e.scratch[:last], int64(len(s))))
	copy(e.scratch[last:], `:"`)
	last += 2
	for len(s) != 0 {
		if asciiPrintNonQuote(s[0]) {
			e.Write(e.scratch[:last])
			last = 0
			cutoff := 1
			for cutoff < len(s) && asciiPrintNonQuote(s[cutoff]) {
				cutoff++
			}
			e.WriteString(s[:cutoff])
			s = s[cutoff:]
			continue
		}
		if last == len(e.scratch)-4 {
			e.Write(e.scratch[:last])
			last = 0
		}
		ch := e.nonAscii(s[0])
		copy(e.scratch[last:], ch)
		last += len(ch)
		s = s[1:]
	}
	if last+1 == len(e.scratch) {
		e.Write(e.scratch[:last])
		last = 0
	}
	copy(e.scratch[last:], `";`)
